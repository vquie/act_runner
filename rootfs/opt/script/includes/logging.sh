...
function logErr() {
    ...
    [[ "${_LOG_LEVEL_ID}" -ge 2 ]] && logMsg "[ERROR] ${_SCRIPT:?} - $1" || return 0
...
function logWarn() {
    ...
    [[ "${_LOG_LEVEL_ID}" -ge 3 ]] && logMsg "[WARN] ${_SCRIPT:?} - $1" || return 0

}

function logInfo() {

    [[ "${_LOG_LEVEL_ID}" -ge 5 ]] && logMsg "[INFO] ${_SCRIPT:?} - $1" || return 0

}

function logDbg() {

    [[ "${_LOG_LEVEL_ID}" -ge 6 ]] && logMsg "[DEBUG] ${_SCRIPT:?} - $1" || return 0
